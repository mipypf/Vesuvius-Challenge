...
COMMENT = """2.5D + 3DCSN + no_mixup_epochs + channel shift
             ...
             + backbone lr option
             + other augmentation + grid_cutout + fbeta_loss + multi_scale
             """
...
def grid_cutout(
    ...
    if np.random.rand() < prob:
        ...
        fill_val = (
            volume.sum() - volume[:, cut_y : cut_y + cut_h, cut_x : cut_x + cut_w].sum()
        ) / (
            volume.numel()
            - volume[:, cut_y : cut_y + cut_h, cut_x : cut_x + cut_w].numel()
        )
        volume[:, cut_y : cut_y + cut_h, cut_x : cut_x + cut_w] = fill_val
...
class InkDetModelMS(nn.Module):
    def __init__(
        ...
        self.conv_proj = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Conv2d(
                        num_features[i + (len(num_features) - 3)], 512, 1, stride=1
                    ),
                    nn.BatchNorm2d(512),
                    nn.ReLU(),
                )
    ...
    def forward_image_feats(self, img):
        ...
        img_feat = self.encoder(img)[-3:]
        if self.output_fmt == "NHWC":
            img_feat = [imf.permute(0, 3, 1, 2).contiguous() for imf in img_feat]
        img_feat = [
            self.conv_proj[i](img_feat[i]) for i in range(3)
        ]  # [(bs * groups_3d, 512, h * 4, w * 4), (bs * groups_3d, 512, h * 2, w * 2), (bs * groups_3d, 512, h, w)]
        img_feat = torch.cat(
            [
                F.interpolate(
                    imf,
                    size=img_feat[-1].shape[-2:],
                    mode="bilinear",
                    align_corners=True,
                )
                for imf in img_feat
...
class InkDetLightningModelMS(pl.LightningModule):
    def __init__(
        self,
    ...
    def add_model_specific_args(
        ...
        parser = parent_parser.add_argument_group("InkDetLightningModelMS")
        parser.add_argument(
            "--model_name",
            default="resnet34",
            type=str,
...
def get_args() -> argparse.Namespace:
    ...
    parser = InkDetLightningModelMS.add_model_specific_args(parent_parser)
