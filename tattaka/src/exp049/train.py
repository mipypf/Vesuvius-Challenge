...
COMMENT = """2.5D + 3DCSN + no_mixup_epochs + channel shift
             ...
             + sharpmarker + ema + label smoothing + fix seed + heavy aug
...
class InkDetModel(nn.Module):
    ...
    def forward_image_feats(self, img):
        ...
        # median = img.reshape(img.shape[0], -1).median(dim=1)[0][:, None, None, None] + 1e-6
        # mad = (
        #     (img - medianm)
        #     .abs()
        ...
        # ) + 1e-6
        # print(mad[4], median[4], (img - median)[4].abs().mean())
        # img = (img - median) / mad
