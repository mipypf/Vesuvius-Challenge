...
EXP_ID = "011"
COMMENT = "2.5D baseline + short iteration + bce loss + split + 49 layer + fixed length dataset + percentile threshold + fbeta_loss"
...
class FBetaLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        ctp = (preds_prob * targets).sum()
        cfp = (preds_prob * (1 - targets)).sum()
...
class InkDetLightningModel(pl.LightningModule):
    ...
    def __build_model(
        ...
        self.criterions = {"bce": nn.BCEWithLogitsLoss(), "fbeta": FBetaLoss(beta=0.5)}

    def calc_loss(self, outputs, labels):
        losses = {}
        losses["bce"] = self.criterions["bce"](outputs["logits"], labels["targets"])
        losses["fbeta"] = self.criterions["fbeta"](outputs["logits"], labels["targets"])
