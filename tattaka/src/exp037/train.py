...
EXP_ID = "037"
COMMENT = "2.5D + 3DCSN + no_mixup_epochs + channel shift + stochastic depth + mid_layer option + groups option + manifold mixup + cutmix + normal mixup + dense head"
...
class InkDetModel(nn.Module):
    def __init__(
        ...
        self.head = nn.Sequential(
            nn.Conv2d(
                channels[-1] * ((65 - start_z + end_z) // in_chans),
                512,
                1,
            ),
    ...
    def forward_image_feats(self, img):
        ...
        img_feat = self.encoder(img)[-1]  # (bs * groups_3d, 512, h, w)
        _, ch, h, w = img_feat.shape
        # print("forward", groups_3d * ch, groups_3d, ch)
        img_feat = img_feat.reshape(bs, groups_3d, ch, h, w).reshape(
            bs, -1, h, w
        )  # (bs, ch * groups_3d, h, w)
        return img_feat
        ...
        if (
            ...
            if np.random.rand() < 0.5:
                ...
                if np.random.rand() < 0.5:
                    ...
                    bbx1, bby1, bbx2, bby2 = rand_bbox(volume.size(), self.mixupper.lam)
