...
COMMENT = """2.5D + 3DCSN + no_mixup_epochs + channel shift
             ...
             + shapemarker + ema + label smoothing + cls_head
             """
...
class InkDetModel(nn.Module):
    ...
    def forward_image_feats(self, img):
        ...
        img_feat_cls = torch.cat(
            [img_feat.mean([2, 3, 4]), img_feat.amax([2, 3, 4])], 1
        )
        img_feat = torch.cat([img_feat.mean(2), img_feat.amax(2)], 1)
        return img_feat, img_feat_cls
    ...
    def forward(
        ...
        img: (bs, ch, h, w)
        """
        img_feat, img_feat_cls = self.forward_image_feats(img)
        return self.head(img_feat), self.cls_head(img_feat_cls)
...
class InkDetLightningModel(pl.LightningModule):
    ...
    def calc_loss(self, outputs: torch.Tensor, labels: torch.Tensor):
        ...
        losses["bce_cls"] = self.criterions["bce"](
            outputs["logits_cls"],
            labels["targets"].amax([2, 3]),
        )
        losses["loss"] = losses["bce"] + losses["bce_cls"] * 0.5
        return losses
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (
            ...
            if np.random.rand() < 0.5:
                img_feat, img_feat_cls = self.model.forward_image_feats(volume)
                img_feat = self.mixupper.lam * img_feat + (
                    1 - self.mixupper.lam
                ) * img_feat.flip(0)
                img_feat_cls = self.mixupper.lam * img_feat_cls + (
                    1 - self.mixupper.lam
                ) * img_feat_cls.flip(0)
                outputs["logits"] = self.model.head(img_feat)
                outputs["logits_cls"] = self.model.cls_head(img_feat_cls)
            else:
                volume = self.mixupper.lam * volume + (
                    1 - self.mixupper.lam
                ) * volume.flip(0)
                outputs["logits"], outputs["logits_cls"] = self.model(volume)
        else:
            if (
                np.random.rand() < 0.5
                and self.current_epoch < self.trainer.max_epochs - self.no_mixup_epochs
    ...
    def validation_step(self, batch, batch_idx):
        ...
        outputs["logits"], outputs["logits_cls"] = self.model_ema.module(volume)
