...
EXP_ID = "027"
COMMENT = "2.5D + 3DCSN + no_mixup_epochs + channel shift + stochastic depth + mid_layer option + groups option + input BN"
...
class InkDetModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.in_bn = nn.BatchNorm3d(in_chans)
    ...
    def forward(
        ...
        img = img.reshape((bs, groups_3d, self.in_chans, h, w)).permute(0, 2, 1, 3, 4)
        img = self.in_bn(img)
        img = img.permute(0, 2, 1, 3, 4).reshape(bs * groups_3d, self.in_chans, h, w)
