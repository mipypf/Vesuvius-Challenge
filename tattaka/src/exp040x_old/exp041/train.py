...
EXP_ID = "041"
COMMENT = "2.5D + 3DCSN + no_mixup_epochs + channel shift + stochastic depth + mid_layer option + groups option + manifold mixup + cutmix + normal mixup + shapemarker + cls_head"
...
class InkDetModel(nn.Module):
    ...
    def forward(
        ...
        seg_pred = self.head(torch.cat([img_feat.mean(2), img_feat.amax(2)], 1))
        cls_pred = self.cls_head(
            torch.cat([img_feat.mean([2, 3, 4]), img_feat.amax([2, 3, 4])], 1)
        )
...
class InkDetLightningModel(pl.LightningModule):
    ...
    def calc_loss(self, outputs: torch.Tensor, labels: torch.Tensor):
        ...
        losses["bce_cls"] = self.criterions["bce"](
            outputs["logits_cls"], labels["targets"].amax([2, 3])
        )
        losses["loss"] = losses["bce"] + losses["bce_cls"]
        return losses
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (
            ...
            if np.random.rand() < 0.5:
                ...
                outputs["logits"] = self.model.head(
                    torch.cat([img_feat.mean(2), img_feat.amax(2)], 1)
                )
                outputs["logits_cls"] = self.model.cls_head(
                    torch.cat([img_feat.mean([2, 3, 4]), img_feat.amax([2, 3, 4])], 1)
                )
            else:
