...
EXP_ID = "006"
COMMENT = "2.5D baseline + short iteration + bce + dice loss"
...
class PatchDataset(Dataset):
    ...
    def __len__(self):
        if self.train:
            return len(self.volume_paths) // 5
        else:
...
class InkDetLightningModel(pl.LightningModule):
    ...
    def __build_model(
        ...
        self.criterions = {"bce": nn.BCEWithLogitsLoss(), "dice": DiceLoss()}

    def calc_loss(self, outputs, labels):
        losses = {}
        losses["bce"] = self.criterions["bce"](outputs["logits"], labels["targets"])
        losses["dice"] = self.criterions["dice"](outputs["logits"], labels["targets"])
        losses["loss"] = losses["bce"] + losses["dice"]
        return losses
