...
EXP_ID = "025"
COMMENT = "2.5D + 3DCSN + no_mixup_epochs + channel shift + stochastic depth + layernorm on 3d layer"
...
class ResidualConv3D(nn.Module):
    def __init__(
        ...
        self.bn1 = nn.LayerNorm(mid_channels)
        ...
        self.bn2 = nn.LayerNorm(mid_channels)
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.bn1(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        x = self.act1(x)

        x = self.conv2(x)
        x = self.bn2(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        x = self.act2(x)

        x = self.conv3(x)
        x = self.bn3(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
