...
EXP_ID = "038"
COMMENT = "2.5D + 3DCSN + no_mixup_epochs + channel shift + stochastic depth + mid_layer option + groups option + manifold mixup + cutmix + normal mixup + shapemarker + channel shuffle "
...
class InkDetModel(nn.Module):
    ...
    def forward_image_feats(self, img):
        ...
        if self.training and np.random.rand() < 0.5:
            ch_arr = list(range(img.shape[1]))
            ch_arr = [random.sample(ch_arr, len(ch_arr)) for _ in range(img.shape[0])]
