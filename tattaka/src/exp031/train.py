...
EXP_ID = "031"
COMMENT = "2.5D + 3DCSN + no_mixup_epochs + channel shift + stochastic depth + mid_layer option + groups option + manifold mixup + mean frame_v2"
...
class InkDetModel(nn.Module):
    ...
    def forward_image_feats(self, img):
        ...
        img = (img - mean) / std
        bs, ch, h, w = img.shape
        img = img.reshape((bs, -1, self.subsample, h, w)).mean(2)
        bs, ch, h, w = img.shape
