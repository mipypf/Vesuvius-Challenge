...
EXP_ID = "030"
COMMENT = "2.5D + 3DCSN + no_mixup_epochs + channel shift + stochastic depth + mid_layer option + groups option + manifold mixup + mean frame"
...
class InkDetModel(nn.Module):
    ...
    def forward_image_feats(self, img):
        ...
        img = (
            img.reshape((bs, groups_3d, self.in_chans, h, w))
            .reshape(bs * groups_3d, self.in_chans, h, w)
            .mean(1, keepdim=True)
        )
