...
EXP_ID = "034"
COMMENT = "2.5D + 3DCSN + no_mixup_epochs + channel shift + stochastic depth + mid_layer option + groups option + manifold mixup + 2.5d branch"
...
class InkDetModel(nn.Module):
    ...
    def forward_image_feats(self, img):
        ...
        img = img.reshape((bs, groups_3d, self.in_chans, h, w)).reshape(
            bs * groups_3d, self.in_chans, h, w
        )
        img_feats = self.encoder(img)
        img_feat_3d = None
        for i, j in enumerate(np.arange(-len(self.conv3d_stages) - 1, -1)):
            img_feat = img_feats[j]  # (bs * groups_3d, ch, h, w)
            _, ch, h, w = img_feat.shape
            img_feat = img_feat.reshape(bs, groups_3d, ch, h, w).transpose(
                1, 2
            )  # (bs, ch, groups_3d, h, w)
            if img_feat_3d is not None:
                img_feat = (img_feat + img_feat_3d) / 2
            img_feat_3d = self.conv3d_stages[i](img_feat)
        _, ch, h, w = img_feats[-1].shape
        img_feat = (
            img_feats[-1].reshape(bs, groups_3d, ch, h, w).transpose(1, 2) + img_feat_3d
        ) / 2
