...
EXP_ID = "007"
COMMENT = "2.5D baseline + short iteration + bce loss + split"
...
class InkDetModel(nn.Module):
    ...
    def forward(
        ...
        """
        bs, ch, h, w = img.shape
        img = img.reshape((bs, 5, 13, h, w)).reshape(bs * 5, 13, h, w)
        img_feat = self.encoder(img)[-1]  # (bs * 13, ch, h, w)
        _, ch, h, w = img_feat.shape
        img_feat = img_feat.reshape(bs, 5, ch, h, w)
...
class InkDetLightningModel(pl.LightningModule):
    ...
    def __build_model(
        ...
        self.criterions = {"bce": nn.BCEWithLogitsLoss()}
