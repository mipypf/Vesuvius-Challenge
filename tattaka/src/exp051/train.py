...
class InkDetModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.first_bn = nn.BatchNorm2d(65 - start_z + end_z)
    ...
    def forward_image_feats(self, img):
        ...
        # median = img.reshape(img.shape[0], -1).median(dim=1)[0][:, None, None, None]
        # mad = (
        #     (img - median)
        #     .abs()
        #     .reshape(img.shape[0], -1)
        #     .median(dim=1)[0][:, None, None, None]
        # ) + 1e-6
        # img = (img - median) / mad
        ...
        optimizer_parameters = [
            ...
            },
            {
                "params": [
                    ...
                    + list(self.model.first_bn.named_parameters())
                    if any(nd in n for nd in no_decay)
                ],
                "weight_decay": 0,
                "lr": self.lr,
