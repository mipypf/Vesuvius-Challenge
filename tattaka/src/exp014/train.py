...
class ResidualConv3D(nn.Module):
    def __init__(
        ...
        self.se = SEModule3D(out_channels)
...
class InkDetModel(nn.Module):
        ...
        for i in range(3):
            ...
            conv3d.append(
                ResidualConv3D(
                    channels[-2],
                    channels[-2],
                    channels[-1] if i == 2 else channels[-2],
                    stride,
                )
            )
    ...
    def forward(
        ...
        img_feat = self.encoder(img)[-2]  # (bs * 7, ch, h, w)
        _, ch, h, w = img_feat.shape
        img_feat = img_feat.reshape(bs, 7, ch, h, w).transpose(
            1, 2
        )  # (bs, ch, 7, h, w)
        img_feat = self.conv3d(img_feat)  # (bs, ch, 7, h, w)
