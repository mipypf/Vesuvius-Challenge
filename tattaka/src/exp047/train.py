...
COMMENT = """
          ...
          shapemarker + ema + label smoothing + ir supervision
          """
...
class PatchDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, int, int]:
        ...
        for i in range(self.image_size[0] // self.PATCH_SIZE):
            for j in range(self.image_size[1] // self.PATCH_SIZE):
                ...
                ir_path = os.path.join(
                    data_prefix, data_source, f"ir/ir_{y + i}_{x + j}.npy"
                )
        ...
        ir /= 255.0
        aug = self.transforms(image=volume, mask=label, ir=ir)
        volume = aug["image"]
        ir = aug["ir"].mean(0)[None, :]
        label = aug["mask"][None, :]
        if self.train and np.random.rand() < 0.5:
            volume = volume.flip(0)
        return (volume, label, x, y, ir)
...
class InkDetModel(nn.Module):
    ...
    def forward(
        ...
        pred = self.head(img_feat)
        pred_ir = torch.sigmoid(self.ir_head(img_feat))
        return pred, pred_ir
...
class InkDetLightningModel(pl.LightningModule):
    ...
    def calc_loss(self, outputs: torch.Tensor, labels: torch.Tensor):
        ...
        losses["loss"] = losses["bce"] + losses["l1"] * 0.1
        return losses
    ...
    def training_step(self, batch, batch_idx):
        ...
        volume, label, _, _, label_ir = batch
        if (
            ...
            if np.random.rand() < 0.5:
                ...
                outputs["pred_ir"] = self.model.ir_head(img_feat)
            else:
                volume = self.mixupper.lam * volume + (
                    1 - self.mixupper.lam
                ) * volume.flip(0)
                outputs["logits"], outputs["pred_ir"] = self.model(volume)
        else:
            if (
                np.random.rand() < 0.5
                and self.current_epoch < self.trainer.max_epochs - self.no_mixup_epochs
                ...
                label_ir[:, :, bbx1:bbx2, bby1:bby2] = label.flip(0)[
                    :, :, bbx1:bbx2, bby1:bby2
                ]
            outputs["logits"], outputs["pred_ir"] = self.model(volume)
        loss_target["targets"] = label
        loss_target["targets_ir"] = label_ir
        ...
        if (
            ...
            loss_target["targets_ir"] = loss_target["targets_ir"].flip(0)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        volume, label, x, y, label_ir = batch
        outputs["logits"], outputs["pred_ir"] = self.model_ema.module(volume)
